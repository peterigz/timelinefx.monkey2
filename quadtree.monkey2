...
#Rem
	...
	it's moved outside of its containing _tlQuadTreeNode_.

	When I say object, I mean a _tlBox_, which is a simple axis aligned bounding box type that can be added to the quadtree, the more complex _tlCircle_, _tlLine_ and
	_tlPolygon_ which extend _tlBox_ can also be added, but the quadtree will only concern itself with bounding boxes when a query is made on the
	quadtree.

	Using the quadtree is simple enough, create a new quadtree with whatever dimensions you want and then use _AddBox_ to add bounding boxes to it. In
	your main loop you might want to put _RunQuadtreeMaintenance_ which tidies up the quadtree by finding empty partitions and deleting them. Of course
	...
	you can query the quadtree by simply calling either _QueryQuadtreeArea_ or _QueryQuadtreeBox_ which will run a callback function of your choice to perform your
	specific tasks on them. There's also queries available to check for objects within a radius by using, _QueryQuadtreeRange_ and _QueryQuadtreeCircle_, and also
	for lines and rays using _QueryQuadtreeEdge_, _QueryQuadtreeLine_ and _QueryQuadtreeRay_.

	Implementing this quadtree within your game will probably involve including _tlBox, _tlCircle, _tlLine or _tlPolygon as a field within your entity/actor etc types.
	When your actors move about, just make sure you update the position of the Box as well using _SetBoxPosition_ or _MoveBox_. When this happens all the necessary updating
	...
	No, onced a node is partioned and objects moved down, they're there to stay, however if you _RunQuadtreeMaintenance_ then empty nodes will be unpartitioned. I
	didn't think it was worth the overhead to worry about moving objects back up the tree again.

	#####What collision checking is done when calling, for example, _QueryQuadtreeArea_?
	...
	collision in the case of _QueryQuadtreeRange_. Once you've found those objects you can then go on and do more complex collision checking such as poly->poly. If
	...
	This applies to both _QueryQuadtreeArea_ and _QueryQuadtreeRange_ functions, but as mentioned before, it will only cull according to bounding boxes, you'll have
	...
	When you run a _QueryQuadtreeArea_ (of any type) you can pass an object that will be passed through to the callback function. So the call back function
	...
	tlBoundaries have a field called collisiontype which you can find out by calling _GetCollisionType_. This will return either tlBOX_COLLISION,
	tlCIRCLE_COLLISION, tlPOLY_COLLISION or tlLINE_COLLISION. The chances are though, that you won't need to know the type, as a call to _CheckCollision_
	will automatically determine the type and perform the appropriate collision check.

	#####Can I have more then one quadtree

	Yes you can create as many quadtrees as you want, however, bear in mind that a _tlBox_ can only ever exist in 1 quadtree at a time. In most cases, with
	...
	which can be defined according to how you need it to be optimised. See _SetQuadtreeLayerConfig_. So you could put a load of enemy objects onto one Layer
...
Class tlQuadTree
	...
	#Rem
		...
		@param line A _tlLine_ that will be used to define the area that is searched
		@param Data any object object to pass to the query
		@param onFoundObject A tlQuadTreeEvent interface that contains the event that will be called for any objects found in the area
		@param Layer Array of layers in the quadtree that you want to search
